#pragma once
// The menu's c-string
const char menu[] = "<E>dit / <M>ove / <R>eplace / <D>raw / <C>lear / <L>oad / <S>ave / <Q>uit: ";
const char menuTwoText[] = "<F>ill / <L>ine / <B>ox / <N>ested Boxes / <T>ree / <M>ain Menu: ";

const int MAXROWS = 22;
const int MAXCOLS = 80;
const int MAXROWSEDGE = MAXROWS - 2;
const int MAXCOLSEDGE = MAXCOLS - 2;
const int BUFFERSIZE = 20;
const int FILENAMESIZE = 255;

// Row location of the menu and it's length + space for input.
const int menuRow = MAXROWS + 2;
const int menuLength = strlen(menu) + 15;

// Row location of menuTwo and it's length + space for input.
const int menuTwoRow = MAXROWS + 2;
// 13 for the "<A>nimate "
const int menuTwoLength = strlen(menuTwoText) + 15+10;

// Canvas Dimensions
const int rowCenter = MAXROWS / 2;
const int colCenter = MAXCOLS / 2;

// Full size of any canvas array
const int arrSize = MAXROWS * MAXCOLS;
